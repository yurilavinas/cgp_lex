...
class ModelCGP(ModelML, Observable):
    def __init__(self, generations, strategy, parser, select_method = None): # changed
        super().__init__()
        self.generations = generations
        self.strategy = strategy
        self.parser = parser
        self.callbacks = []
        self.select_method = select_method # changed
        # changed
        if select_method == None:
            self.select_method = 'Best'
        # changed   
    ...
    def fit(
        ...
        while not genetic_algorithm.is_satisfying():
            ...
            # changed
            _, fitness, idx = self.strategy.population.get_best_individual()
            if overall_best_fitness > fitness:
                overall_best_fitness = fitness
                with open("results/overall_best_" + self.select_method + ".txt", "a") as f:
                    csvwriter = csv.writer(f)
                    csvwriter.writerows([[self.strategy.population._fitness['fitness'][idx], self.strategy.population._fitness['act_nodes'][idx], genetic_algorithm.current_generation]])
            with open("results/current_best_" + self.select_method + ".txt", "a") as f:
                csvwriter = csv.writer(f)
                csvwriter.writerows([[self.strategy.population._fitness['fitness'][idx], self.strategy.population._fitness['act_nodes'][idx], genetic_algorithm.current_generation]])
            # changed
