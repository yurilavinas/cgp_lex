...
class StackerSum(KartezioStacker):
    ...
    def stack(self, Y: List):
        stack_array = np.array(Y)
        stack_sum = np.sum(stack_array, axis=0, dtype=np.uint16)
        return np.clip(stack_sum, 0, 255).astype(np.uint8)
...
class StackerMax(KartezioStacker):
    ...
    def __init__(self, name="max_stacker", symbol="MAX", arity=1, threshold=4):
        super().__init__(name, symbol, arity)
        self.threshold = threshold
